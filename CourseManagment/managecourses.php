    ...
    <table class="table">
      ...
      <tbody>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          $stmt2 = $connect->prepare("SELECT UserDetails.username, RelationalTable.TIMESTAMP FROM RelationalTable JOIN UserDetails ON RelationalTable.UserID = UserDetails.userID WHERE RelationalTable.CourseID = ?");
          ...
          if ($result2->num_rows === 0) {
            ...
            while ($row2 = mysqli_fetch_assoc($result2)) {
              echo $row2['username'] . ' - Assigned on: ' . $row2['TIMESTAMP'] . '<br>';
            }
          }
          $stmt2->close();
          echo '</div>';
